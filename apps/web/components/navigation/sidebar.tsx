...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 h-screen bg-white/80 backdrop-blur-[20px] text-[#24283A] flex-col shrink-0 border-r border-[#D0D7E3]/70 shadow-[0_2px_12px_rgba(31,38,135,0.04)] select-none">
      {/* Brand Header */}
      <div className="h-[72px] flex items-center gap-3 px-6 border-b border-[#D0D7E3]/70">
      ...
      <div className="px-5 py-2.5 border-b border-[#D0D7E3]/60 bg-white/60 backdrop-blur-xs flex items-center justify-between">
            ...
            <Link
              ...
              className={`flex items-center justify-between px-3.5 py-2.5 text-xs font-semibold rounded-[11px] transition-all ${
                isActive
                  ...
                  : "text-[#464B5E] hover:text-[#24283A] hover:bg-white/70"
              }`}
            >
...
export function MobileSidebar({
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden flex">
      ...
      <div
        onClick={onClose}
        className="fixed inset-0 bg-[#24283A]/60 backdrop-blur-xs transition-opacity animate-in fade-in duration-200"
      />

      {/* Drawer */}
      <aside className="relative w-72 max-w-[85vw] h-full bg-white/90 backdrop-blur-xl text-[#24283A] flex flex-col shrink-0 border-r border-white/90 shadow-2xl z-10 animate-in slide-in-from-left duration-250 select-none">
        {/* Drawer Header */}
        <div className="h-[72px] flex items-center justify-between px-5 border-b border-[#D0D7E3]/70">
          <Link href="/" onClick={onClose} className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-[11px] bg-gradient-to-r from-[#7779D8] to-[#5052C9] flex items-center justify-center text-white font-bold shadow-[0_2px_8px_rgba(80,82,201,0.25)]">
              <ShieldCheck className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="text-base font-heading font-extrabold tracking-[-0.02em] text-[#24283A] flex items-center gap-1 leading-tight">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-white/80 hover:bg-[#E5E4EE] text-[#464B5E] hover:text-[#24283A] flex items-center justify-center transition-colors cursor-pointer border border-[#D0D7E3]/60"
          ...
          {navigationItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onClose}
                className={`flex items-center justify-between px-3.5 py-3 text-xs font-semibold rounded-[11px] transition-all min-h-[44px] ${
                  isActive
                    ? "bg-gradient-to-r from-[#7779D8] to-[#5052C9] text-white shadow-[0_2px_10px_rgba(80,82,201,0.22)]"
                    : "text-[#464B5E] hover:text-[#24283A] hover:bg-white/70"
                }`}
